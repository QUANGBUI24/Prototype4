...
class UMLClassBox(QtWidgets.QGraphicsRectItem):
    ...
    def mouseMoveEvent(self, event):
        ...
        if self.is_resizing and self.current_handle is not None:
            ...
            # Calculate the required minimum size to avoid overlapping text
            min_width = max(self.class_name_text.boundingRect().width() + 20, 50)
            min_height = (self.fields_label.y() + self.fields_label.boundingRect().height() +
                       ...
                       self.methods_text.boundingRect().height() + 40)  # Adjust padding

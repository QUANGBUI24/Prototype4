...
class MainWindow(QMainWindow, Observer):
    ...
    def click_save_as(self):
        file_name, _ = QFileDialog.getSaveFileName(self, "Save As", "", "All Files (*);; Python Files (*.py)")
        if file_name:
            self.label.setText(str(file_name))
        self.interface.save()
    def click_open(self):
        file_name, _ = QFileDialog.getSaveFileName(self, "Open", "", "All Files (*);; Python Files (*.py)")
        if file_name:
            self.label.setText(str(file_name))
